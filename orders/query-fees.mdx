---
title: 'Query Order Fees'
description: 'Retrieve transaction fees for your payments.'
icon: 'comment-dollar'
---

Transaction fees vary depending on the selected payment method. Before initiating a payment, use our [fees API]() to query and present the customer with an accurate fee breakdown.

Follow these instructions to query fees before initiating a payment:

1. Encrypt the fee query request. This request should contain the payment type and transaction amount.
2. Send the encrypted request to the fetch fee endpoint.

Use these examples to guide your fee query.

<CodeGroup>

```json Example (Unencrypted)
{
    "amount": 500,
    "payment_option": "CARD"
}
```

```json Example (Encrypted)
{
    "data": ""
}
```

```json 200 OK
{
    "data": {
        "fee": 80.00,
        "amount": 500.0,
        "subsidiary_fee": 80.00,
        "customer_fee": 0.0000000,
        "total_charged_amount": 500.0000000,
        "payment_option": "CARD"
    },
    "status": "success",
    "message": "Operation successful"
}
```

</CodeGroup>